...
with open(CSV_PATH_DIFFICULTS) as in_file:
    ...
    for row in data_reader:
        if row [0]:
            name = row[0]
            Difficulty.objects.create(name = row[0])
...
with open(CSV_PATH_SUBCATEGORIES) as in_file:
    ...
    for row in data_reader:
        if row [0]:
            name        = row[0]
            category_id = row[1]
            Subcategory.objects.create(name = row[0], category_id = row[1])
...
with open(CSV_PATH_LECTURES) as in_file:
    ...
    for row in data_reader:
        if row [0]:
            ...
            Lecture.objects.create(name = row[0], price = row[1], discount_rate = row[2], user_id = row[5], difficulty_id = row[6], subcategory_id = row[7])